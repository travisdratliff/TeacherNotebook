...
struct TeacherSettingsView: View {
    ...
    func appendWeight() {
        ...
        if let color = resolved {
            ...
            SwiftDataManager.insert(gradingWeight, to: modelContext)
        }
        title = ""
        percentage = nil
    ...
    func onboard() {
        for gradingPeriod in gradingPeriods {
            SwiftDataManager.insert(gradingPeriod, to: modelContext)
        }
        for weight in gradingWeights {
            SwiftDataManager.insert(weight, to: modelContext)
        }
    }
}
